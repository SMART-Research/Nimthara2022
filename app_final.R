    ...
    sidebarLayout(
        ...
        mainPanel(
          tabsetPanel(type="tab",
                      ...
                      tabPanel("Overview",
                               verbatimTextOutput("summary"),plotOutput("structureplot")),
                      ...
                      tabPanel("References",br(),a("Vector input in  R shiny",href="https://stackoverflow.com/questions/36949769/how-to-plot-uploaded-dataset-using-shiny"),br(),
                               a("Passing data into drop-down input selector",href="https://stackoverflow.com/questions/35069404/r-shiny-passing-data-into-drop-down-input-selector"),br(),
                               a("Get user to select the variables & plot with ggplot",href="https://stackoverflow.com/questions/53113065/r-shiny-read-data-file-get-user-to-select-variable-plot-with-ggplot"),br(),
                               a("Interactive visualization in R shiny",href="https://bookdown.org/ihaddaden_fodeil/shiny_book-master/reactivity.html"),br(),
                               a("Input widgets in R shiny",href="https://www.youtube.com/watch?v=HPZSunrSo5M")))
...
server <- function(input, output) {
   ...
   output$structureplot<-renderPlot({
       vis_dat(data())+theme(axis.text=element_text(size=14))
   })
  ...
  output$hist<-renderPlot({
      data<-data()
      data_nw<-data[,input$x]
     
      ggplot(data,aes(x=data_nw))+geom_histogram(bins=input$bins,fill = "#d11141")+xlab(input$x)+labs(title="One Quantitative Variable")
  })
